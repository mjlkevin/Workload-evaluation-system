...
// 批次 6a：AI 工具清单（GET /system/ai-tools）夹具。
// 字段与 apps/api 运行时 ToolRegistry 派生的条目一一对应，9 个工具照抄代码注册表。
export const mockAiTools = [
  ...
  {
    name: 'list_tools',
    description: '按意图或类别发现当前可用的工具，返回匹配工具的说明书。需要未直接提供的工具时先调用本工具发现，再按说明书调用目标工具。',
    capability: 'estimates:read',
    mutates: false,
    category: 'discovery',
    discoverable: false,

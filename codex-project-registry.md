...
|---|---|---|---|---|---|---|---|
| WorkEvolutionSys / WES 唯一活动目录 | `/Users/kevin/AI/Workload-evaluation-system` | 原 `-agent` linked worktree 已注销，不得再将历史路径作为入口；不得覆盖现有未提交改动 | Web 主线 `ui/V2_PROTOTYPE`；后端主线 `apps/api`；当前集成分支 `codex/role-driven-ai-home-workbench` | `pwd && git status --short --branch && git worktree list --porcelain` | `npm run build:web`; `npm run build:api`; `npm run test:modules`; `npm run test:web`; `npm run test:ai` | 可写，但必须精确保护无关 dirty changes | 入口/边界审计、issue-first 分诊、代码实现、总看板同步、验证复核 |

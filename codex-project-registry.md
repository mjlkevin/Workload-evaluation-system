...
> 用途：Codex 新会话或子代理启动前先读本表，确认正确工作区、禁止路径、验证命令和协作分工。事实以 2026-06-24 本机检查为准；每次执行前仍需运行对应预检命令确认分支和 dirty 状态。
...
|---|---|---|---|---|---|---|---|
| WES Agent / WorkEvolutionSys | `/Users/kevin/AI/Workload-evaluation-system-agent` | `/Users/kevin/AI/Workload-evaluation-system` 仅历史追溯；不得作为 WES 新主线 | Web 主线 `ui/V2_PROTOTYPE`；后端主线 `apps/api`；当前观察为 `main...origin/main [ahead 5]` 且有大量既有 dirty changes | `pwd && git status --short --branch` | `npm run build:web`; `npm run build:api`; 按改动选择 `npm run test:modules` / `npm run test:web` / `npm run test:harness -w apps/api` | 可写，但必须保护无关 dirty changes | 入口/边界审计、需求池去重、代码实现、总看板同步、验证复核 |
| WES legacy worktree | `/Users/kevin/AI/Workload-evaluation-system` | 不得恢复 `apps/web` 或把旧工作区作为 WES 主线 | 当前观察为 `codex/role-driven-ai-home-workbench`；只用于历史差异核对 | `git status --short --branch` | 无默认验证；除非用户明确要求迁移/追溯 | 默认只读 | 历史代码差异核对、迁移证据抽取 |
...
- 不把旧 WES 路径 `/Users/kevin/AI/Workload-evaluation-system` 当作当前 WES 主线。

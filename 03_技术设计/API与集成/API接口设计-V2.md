...
> **实现差异提示**：本文为 V2 设计文档（轻量、可选鉴权、三页面）。**当前实现已演进为 JWT 登录 + 多页签工作台 + 文件持久化 + 团队协同。** 以 `03_技术设计/系统演进/实现与文档对齐说明.md` 为准。本文保留历史设计思路与评审结论。

## 1. 约定

- Base URL：`/api/v1`
- 数据格式：`application/json`
- 鉴权方式：**当前实现为 JWT**（`Authorization: Bearer <token>`）；健康检查等少数路径除外。历史文档「可选鉴权 / 内网关闭」为设计目标，与现网代码不一致处见 §15 与 `03_技术设计/系统演进/实现与文档对齐说明.md`。
- 时间格式：ISO-8601（UTC）
- 分页参数：
  - `page`（默认 1）

...
> **实现现状注记（2026-03）**：除模板/规则/导出外，已增加用户、推荐码、版本记录等**文件型持久化**。细节见 **§9** 与 `03_技术设计/系统演进/实现与文档对齐说明.md`。
...
## 9. 实现补充：已落盘的业务文件（2026-03）

以下路径相对仓库根目录；**非关系型数据库**，便于单机部署。

| 文件/目录 | 用途 |
|-----------|------|
| `config/templates/*.json` | 模板配置（与 §4.1 一致） |
| `config/rules/*.json` | 规则配置（与 §4.2 一致） |
| `config/auth/users.json` | 注册用户、密码哈希、`admin`/`user` 角色 |
| `config/auth/invite-codes.json` | 推荐码生成、使用状态与审计字段 |
| `config/versions/records.json` | 多类型版本记录（实施/资源/需求/开发/总方案等） |
| `exports/`（或运行时配置目录） | 导出产物；下载接口按用户隔离 |

**前端草稿**：总方案、各子评估页状态主要存于浏览器 `localStorage`，key **按用户作用域**隔离（与 V2「无项目持久化」表述并存：服务端无统一业务库，但不等于无客户端状态）。

## 10. R2 演进启动（2026-04）

- 二轮已启动“数据层演进与集成”准备，详见：
  - `03_技术设计/数据模型/R2-数据层演进与集成启动方案.md`
- 当前实现保留文件存储主路径，同时在 WBS 派生模型中增加追溯字段，作为后续持久化与集成的承接点。
- Beta 数据治理补充：
  - 每日备份：`npm run ops:backup:config`
  - 完整性校验：`npm run ops:check:config`【历史说明，已下线】启动校验器随阶段 2 S7（2026-08-31）整链删除——九域 store 迁 PostgreSQL 后 `REQUIRED_FILES` 已清空，实跑只输出 `checked 0 files.`，不再提供任何配置保护
  - 异常/修复日志：`logs/data-anomaly-repair.log`

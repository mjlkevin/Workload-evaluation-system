...
> **实现现状注记（2026-03）**：当前代码已超出本文「纯轻量、无登录、三页面」的最初边界，演进为多页签工作台 + JWT + 文件型用户/推荐码/版本库 + 前端按用户隔离草稿。请以 `03_技术设计/系统演进/实现与文档对齐说明.md` 为补充事实来源；下文保留历史架构与评审结论。
...
- **当前实现**：无 Element Plus / Naive 等重型 UI 依赖包（见 `apps/web/package.json`）；界面以自定义样式与原生控件为主
- **V2 原文**：曾建议 `Element Plus` + `Pinia`；当前以 **Composition API 单文件聚合（`App.vue`）** 为主

...
> **实现现状注记（2026-05）**：当前代码已大幅超出本文「纯轻量、无登录、三页面」的最初边界，演进为多页签工作台 + JWT + 文件型用户/推荐码/版本库 + 前端按用户隔离草稿 + Kimi AI 评估。**本文保留历史架构决策与评审结论，当前实现事实以 `03_技术设计/系统演进/实现与文档对齐说明.md` 为准。** 技术栈差异：本文所述 NestJS/Element Plus/Pinia → 实际为 Express/Next.js/React。
...
> **演进说明**：当前已实现「文件型用户/推荐码/版本记录」与「总方案归档」，属于**轻量持久化**，不等同于 V2 所排除的“项目台账/审批流/企业级 RBAC”。边界以 `03_技术设计/系统演进/实现与文档对齐说明.md` 为准。
...
- `Next.js + TypeScript`（主前端 `ui/V0_SAAS`）
- **当前实现**：无 Element Plus / Naive 等重型 UI 依赖包；界面以轻量组件与自定义样式为主
- **V2 原文**：曾建议 `Element Plus` + `Pinia`；当前按 Next 页面与组件拆分实现

### 4.2 后端

- **当前实现**：`Express + TypeScript`（单服务入口，见 `apps/api/src/main.ts`）
- **V2 原文建议**：`NestJS + TypeScript`（未采用）
- 规则计算采用纯函数 + 策略模式
...
- `web`（前端静态资源 + Nginx）
- `api`（**当前实现：Express 服务**；V2 原文写 NestJS 为历史建议）

# 工作量评估系统 - 项目开发 TODO List

## 0. 项目初始化
- [x] 完成 Excel 原型结构解析（工作表、字段、公式、约束）
- [x] 建立项目规范目录结构（需求/设计/开发/测试/发布/运维）
- [x] 明确 MVP 范围与一期交付边界（计算 + 导出，不做复杂存储）
- [x] 约定文档命名规范与版本号规则（核心设计文档已统一 V2）
- [x] 完成编码前全量文档巡检与一致性校对
- [x] 建立对话流程总结机制（里程碑持续更新）

## 1. 需求与规则梳理
- [x] 固化业务对象清单（模板、分组、估算项、规则、会话）
- [x] 完成需求评审 1（产品目标层）并形成结论
- [x] 完成需求评审 2（业务能力层）并形成结论
- [x] 完成需求评审 3（评估流程层）并形成结论
- [x] 完成需求评审 4（计算规则层）并形成结论
- [x] 完成需求评审 6（接口与技术层）并形成结论（见 `API接口设计-V2.md` §14）
- [x] 完成 Excel 规则标准化（用户数分段、难度系数、多组织系数，含抽取脚本）
- [x] 区分可配置规则与硬编码规则（核心规则改为参数化/规则化）
- [x] 输出需求基线 V1（含非功能需求，见 `01_需求管理/需求基线V1-首发与迭代分层清单.md`）

## 2. 架构设计（当前进行中）
- [x] 输出总体架构设计初稿（前端/后端/数据/规则引擎/部署）
- [x] 明确技术栈与版本（前端、后端、数据库）
- [x] 完成数据模型设计（精简存储，文件化为主）
- [x] 完成 API 设计（估算、模板、规则、导出）
- [x] 完成权限模型设计（admin/operator 最小 RBAC）
- [x] 完成轻量化架构收敛（移除项目/快照主流程）
- [x] 完成核心文档 Mermaid 可视化补充（架构/模型/API/实施）

## 3. 产品与交互设计
- [x] 完成需求评审 5（页面与交互层）并形成结论
- [x] 输出前端视觉规范（Calendly 风格 + 本地参考图）
- [x] 设计信息架构（页面导航与功能分区）
- [x] 输出估算录入页原型（模板分组/单项可视化勾选 + 实时回算）
- [x] 输出规则配置页原型（分段规则、系数规则，只读摘要面板）
- [x] 输出结果页原型（总计、增量拆解、分组/单项明细、requestId/JSON复制）
- [x] 需求变更：用户端非黑盒录入（展示 `group -> item` 全量结构并支持逐项勾选/反勾选）

## 4. 开发实现（MVP）
- [x] 初始化代码仓（已本地初始化 Git、接入 GitHub 远程并完成 main 首次推送）
- [x] 完成模板/规则导入能力（从 Excel 或 JSON，已提供 API 导入端点）
- [x] 完成估算计算引擎（支持分组小计、增量拆解、requestId追溯）
- [x] 完成规则引擎 pipeline 化（grouping/item/base/orgIncrement 可配置）
- [x] 完成会话态估算流程（不落库，含 `sessions/start` 与会话计算）
- [x] 完成结果导出（Excel/PDF，含幂等键回放）
- [x] 完成最小访问控制（曾用 Header `X-Role` 版 RBAC；**当前已升级为 JWT**，以代码为准，见 `03_技术设计/系统演进/实现与文档对齐说明.md`）
- [x] 输出开发实施清单与工程脚手架方案
- [x] 输出编码前总览报告与开工确认清单

## 5. 测试与质量
- [ ] 编写规则引擎单元测试（覆盖关键公式和边界值）
  - [x] 已完成首版规则引擎单元测试：`npm run test:rules`（用户分段、取整、难度、多组织、分组小计、缺失项校验）
- [ ] 编写接口集成测试（核心 API）
  - [x] 已完成首版 API 集成校验脚本：`npm run test:api:integration`（health/templates/rule-set/calculate/export-idempotency）
- [ ] 执行业务回归（与原 Excel 结果对比）
  - [x] 已完成首版回归脚本与报告：`npm run rules:excel-report`（输出到 `05_测试与质量/测试报告`）
- [ ] 完成性能与稳定性验证（并发计算、导出）

## 6. 发布与部署
- [ ] 准备 Docker 部署方案（单机轻量）
- [ ] 准备环境配置模板（dev/prod）
- [ ] 准备发布与回滚说明
- [ ] 完成首版部署与冒烟验证

### 首轮部署前工作计划（云部署简版）

- [ ] 选型与采购（Beta）
  - [ ] 云主机：Ubuntu 22.04，建议 `2C4G` 起步
  - [ ] 磁盘：建议 `60G SSD`（更稳妥 `80G`）
  - [ ] 域名与备案（按地区要求）
- [ ] 服务器初始化
  - [ ] 创建普通运维用户
  - [ ] 禁用密码登录，仅保留 SSH Key
  - [ ] 防火墙仅开放 `22/80/443`（不暴露 `3000/3001`）
  - [ ] 安装 Docker 与 Docker Compose
- [ ] 项目上云
  - [ ] 拉取代码并准备生产 `.env`（`JWT_SECRET`、`KIMI_API_KEY` 等）
  - [ ] 启动 `docker compose up --build`（api + web）
- [ ] 反向代理与 HTTPS
  - [ ] Nginx/Caddy 反代到 `web:3001`
  - [ ] `/api` 走前端反代到 `api:3000`
  - [ ] 配置 Let's Encrypt 证书
- [ ] 数据持久化与备份
  - [ ] 确认 `config/*` 挂载宿主机目录
  - [ ] 每日备份 `config/*`（压缩 + 保留 7~14 天）
- [ ] 上线验收与运维基线
  - [ ] 健康检查：`/api/v1/health`
  - [ ] 主链路冒烟：登录、版本流转、导出、团队协作
  - [ ] 监控 CPU/内存/磁盘与容器日志
  - [ ] 建立发布脚本（拉代码、重建、重启、回滚）
  - [ ] 建立故障 SOP（证书、磁盘、服务异常）
- [ ] 采购前可先完成（本地预演）
  - [ ] 本地跑通 `docker compose up --build`
  - [ ] 产出生产环境变量清单
  - [ ] 完成“上线检查清单 + 回滚步骤”
  - [ ] 预先确认云厂商与规格

## 7. 交付与运营
- [ ] 输出用户手册与管理员手册
- [ ] 输出试点项目复盘报告
- [ ] 收集优化项并规划 V1.1 迭代
- [ ] 新增“团队协同与角色化工作台”能力专项（未来需求，V2候选）：
  - [ ] 建立团队组织模型（团队、成员、角色、归属关系）
  - [ ] 角色体系首版：`部门负责人`、`实施顾问`、`售前顾问`、`销售顾问`
  - [ ] 支持跨角色组建团队与成员管理（加入/移除/角色调整）
  - [ ] 部门负责人可查看本团队下全部评估方案（列表、状态、负责人、进度）
  - [ ] 支持围绕“总方案”的评审、评论与协同流转（含意见留痕）
  - [ ] 评审协同需保留最小审计字段（谁在何时做了什么）
- [ ] 新增“AI 友好调用”能力专项（Agent-Friendly API）：
  - [ ] P0（MVP，优先两周内完成）
    - [ ] 设计并实现高层估算入口 `POST /api/v1/agent/estimate`（后端托管模板/规则上下文，减少必填参数）
    - [ ] 内置参数补全与容错（userCount/orgCount/difficultyFactor 缺省回填与边界修正）
    - [ ] 输出统一“可解释反馈”结构（`missingFields` / `assumptions` / `nextQuestions`）
    - [ ] 提供最小会话态能力（`sessionId` + `contextSummary`），支持 Agent 多轮追问
    - [ ] 新增 10+ 条 Agent 集成测试样例（缺参、错参、追问补全、成功估算）
    - [ ] W1（接口与后端能力）
      - [ ] 后端：实现 `POST /api/v1/agent/estimate` 与 `POST /api/v1/agent/session/start`
      - [ ] 后端：参数补全器（默认值、边界值、非法值修正）与标准错误映射
      - [ ] 后端：响应结构统一（success / needs_clarification / failed）
      - [ ] 测试：新增单测（补全逻辑、缺参逻辑、冲突参数逻辑）
    - [ ] W2（联调与可用性验收）
      - [ ] 后端：实现 `POST /api/v1/agent/session/:sessionId/continue`
      - [ ] 前端：新增 Agent 调试页（输入自然语言 -> 查看结构化响应）
      - [ ] 测试：完成 10+ 条端到端场景回归（多轮追问、补问收敛、估算成功）
      - [ ] 文档：输出 Agent 调用示例会话（成功路径 + 异常路径）
    - [ ] 角色分工（建议）
      - [ ] 后端负责人：接口设计、补全与校验、会话上下文、可解释响应
      - [ ] 前端负责人：Agent 调试页、可视化反馈面板、会话追踪展示
      - [ ] 测试负责人：场景用例库、稳定性回归、失败分类统计
  - [ ] P1（增强）
    - [ ] 增加意图解析层（自然语言 -> 模板/工作表/条目候选）
    - [ ] 增加“候选项确认”接口（返回 topK 候选 + 置信度 + 解释）
    - [ ] 增加“自动补全建议”接口（面向 Agent 的下一问建议）
    - [ ] 对导出链路增加 Agent 友好返回（结构化下载信息 + 可读说明）
  - [ ] P2（体验优化）
    - [ ] 增加反馈闭环（用户确认结果反哺，优化映射规则）
    - [ ] 建立 Agent 调用质量看板（成功率、补问次数、平均回合数、失败原因）
    - [ ] 沉淀 Agent 契约版本治理（兼容策略、弃用策略、变更公告）

---

## 近期两周重点（建议）
1. 需求规则固化（优先级 P0）
2. 轻量存储方案落地（模板/规则文件化）（优先级 P0）
3. MVP 计算 + 导出主流程闭环（优先级 P0）
4. Docker 单机部署跑通（优先级 P1）

## 近期状态对齐（2026-03）

- [x] 前端已完成多页签评估流程主链路：需求、实施评估、开发评估、资源人天及成本、总览联动
- [x] 已完成“评估方案列表”与方案预览弹窗（关联关系图 + 基本分析 + 子评估摘要）
- [x] 已完成总方案版本（`GL-`）与子版本（`PG-`/`RI-`/`DV-`/`RS-`）关联保存与回填
- [x] 已完成账号体系（注册/登录/JWT）与管理员能力（用户管理、推荐码管理）
- [x] 已完成按用户维度的数据隔离（草稿、导出、下载鉴权）
- [x] 已完成主菜单六模块二级入口（列表/新增）与模块单据列表页骨架（预览/修改/删除/查看历史）
- [x] 已补充模块级删除服务与“当前用户最新单据”聚合适配（含 WBS/评审历史暂不支持提示）
- [x] 已新增“产品线”维度贯通（需求基本信息、实施评估评估信息、实施评估列表列展示）
- [ ] 待完善：部署发布链路（Docker 与环境模板）及运维文档固化

---

## R0 需求分层冻结（2026-04-04）

### 首发（严格 MVP，Freeze List v1）

- [ ] 版本机制核心链路可回归（checkout/checkin/undo/promote/force-unlock）
- [ ] 四大评估主流程可完整演示（需求/实施/开发/资源）
- [ ] 登录鉴权 + 团队权限最小闭环稳定可用
- [ ] 评审页最小闭环可演示（创建/状态变更/评论）
- [ ] 发布可复现（最小 CI + 部署手册 + 环境模板）
- [ ] OpenAPI 与主线路由一致（重点 versions/team/wbs）

### 二轮（集成与数据层）

- [ ] 数据层演进方案与迁移样板（1~2 条关键链路）
- [ ] 外部系统/数据集成骨架与契约定义
- [ ] WBS 从只读派生向可运营能力增强
- [ ] Beta 数据治理三件事（单实例前提）：
  - [ ] 每日自动备份 `config/*`
  - [ ] 启动或定时执行数据完整性校验脚本
  - [ ] 记录数据异常/修复日志，作为迁移决策证据

### 三轮（产品完整度）

- [ ] 评审链路、WBS、标准 PLM 形成完整闭环
- [ ] 跨模块体验一致性与产品细节补齐
- [ ] 数据库接入立项（仅在触发器满足后执行）：
  - [ ] 触发器 A：并发写冲突频繁（每周 >= 2 次人工修复）
  - [ ] 触发器 B：核心查询 P95 超出阈值
  - [ ] 触发器 C：多实例部署/容器扩容需求明确
  - [ ] 触发器 D：审计、统计、权限追溯需求显著增长

### 首发发布门禁（统一口径）

- [ ] `npm run build:web`
- [ ] `npm run build:api`
- [ ] `npm run test:modules`
- [ ] `npm run test:rules`
- [ ] `npm run test:integration`
- [x] 最小 CI 已接入：`.github/workflows/minimal-ci.yml`（构建 + API 测试）

---

## 今日推进（2026-03-30）

- [x] 回归校验通过：`npm run test:modules`、`npm run test:api:team`
- [ ] （历史项）Agent 冒烟 `test:api:agent`、访问日志汇总 `logs:api:report`：未纳入当前根脚本；若重新引入中间件/脚本再勾选
- [x] 团队协同 P0 评审结论：`01_需求管理/需求分析/团队协同-P0需求评审结论.md`
- [x] 团队协同 PRD 草案：`01_需求管理/需求分析/团队协同与角色化工作台-PRD草案.md`
- [x] 团队协同技术方案草图：`03_技术设计/系统架构/团队协同能力-技术方案草图.md`
- [x] 团队协同后端骨架：`apps/api/src/modules/team/*` + `apps/api/src/routes/team.routes.ts`
- [x] 团队协同后端测试：`modules.usecase.test.ts` 新增 3 条（权限/隔离/状态流转）
- [x] 对齐验证通过：`npm run build:api`、`npm run test:modules`
- [x] 团队协同接口契约对齐：`docs/openapi.yaml`（Teams 路径 + 当前错误语义）
- [x] 团队协同联调文档与全链路 curl：`docs/LLM_API_CALLING_GUIDE.md`
- [x] 团队协同自动化冒烟：`npm run test:api:team`（正常 + 异常场景）
- [x] 团队存储并发加固：原子写入 + `version` 乐观并发控制（冲突码 `40909`）

## 里程碑地图（可视化）

### 1) 总体里程碑时间线

```mermaid
timeline
    title 工作量评估系统里程碑地图（V1）
    M1 需求基线 : Excel解析完成
               : 需求评审1/2/3/4/5/6完成
               : 非黑盒录入需求变更纳入基线
    M2 设计冻结 : 架构/模型/API/权限文档V2完成
               : 前端视觉规范完成
    M3 MVP研发  : 代码仓初始化与GitHub打通
               : 计算/导出/下载主链路打通
               : 用户端分组单项勾选可视化落地
    M4 联调测试 : 规则引擎单测
               : 接口集成测试
               : Excel回归对比
    M5 发布上线 : Docker部署
               : 冒烟验证
               : 用户手册与试点复盘
```

### 2) 研发推进甘特图（建议）

```mermaid
gantt
    title V1 开发与交付路线图
    dateFormat  YYYY-MM-DD
    axisFormat  %m/%d

    section 已完成（基线）
    需求评审与架构收敛            :done, a1, 2026-03-01, 8d
    API基线与视觉规范              :done, a2, after a1, 5d
    代码仓初始化与脚手架            :done, a3, after a2, 3d
    非黑盒录入页（分组单项勾选）      :done, a4, after a3, 4d

    section 进行中（P0）
    规则标准化（Excel -> JSON）     :active, b1, 2026-03-24, 7d
    结果页解释增强（分组/增量拆解）    :active, b2, 2026-03-25, 5d
    导出能力完善（Excel/PDF版式）     :active, b3, 2026-03-25, 6d

    section 待开始（P1）
    测试闭环（单测/集成/回归）       :c1, after b1, 6d
    Docker部署与发布说明            :c2, after c1, 4d
    用户手册与试点复盘              :c3, after c2, 4d
```

### 3) 关键路径流转图（从需求到上线）

```mermaid
flowchart LR
    A[需求基线冻结] --> B[规则标准化]
    B --> C[模板/规则配置冻结]
    C --> D[计算引擎稳定]
    D --> E[导出能力稳定]
    E --> F[前后端联调验收]
    F --> G[测试闭环]
    G --> H[Docker部署与冒烟]
    H --> I[试点上线]

    A1[已完成: 非黑盒录入设计] -.支撑.-> F
    A2[已完成: API基线与版本追溯] -.支撑.-> D
    A3[已完成: GitHub版本管理] -.支撑.-> H
```

### 4) 当前任务看板（状态视图）

```mermaid
flowchart TB
    subgraph Done[已完成]
      D1[需求评审1~6]
      D2[架构/数据/API/权限V2]
      D3[前端视觉规范]
      D4[代码仓初始化 + GitHub接通]
      D5[非黑盒录入页（分组单项勾选）]
    end

    subgraph Doing[进行中]
      G1[规则/模板导入后治理（命名、去重、口径校准）]
      G2[Excel回归对比脚本与样本对比]
      G3[导出版式完善]
    end

    subgraph Todo[待开始]
      T1[规则引擎单测]
      T2[接口集成测试]
      T3[Excel回归对比]
      T4[Docker部署与发布]
      T5[用户手册与试点复盘]
    end
```

## 今日推进（2026-04-06）

- [x] 前端导航新增「系统管理」入口（位于用户管理下）
- [x] 新增系统管理页：版本号编码规则配置（按模块配置/禁用/生效，工具栏 + 表格）
- [x] 系统管理页预留“其他管理能力”容器布局（字典、通知、审计占位）
- [x] 新增后端系统管理接口：版本号编码规则列表/配置/生效/禁用（文件持久化）
- [x] 系统管理前端改造为真实 API 联动（由内存态切换为后端持久化）
- [x] 版本创建接入规则引擎：按系统管理“已生效”规则自动生成版本号（前端移除硬编码拼接）
- [x] 补充模块测试：版本号规则生成关键场景（冲突递增/规则未生效/固定编码冲突）
- [x] 全局弹窗支持顶部空白区拖拽（通用 `Dialog` 组件能力）
- [x] 新增“Kimi 智能评估（需求 -> 实施评估预览）”专项（P0）：
  - [x] P0-W1：完成需求页入口与接口契约冻结（`Kimi-help -> Kimi评估`）
  - [x] P0-W1：后端新增预览接口 `POST /api/v1/ai/kimi-assessment/preview`（JWT + 统一响应）
  - [x] P0-W1：完成提示词模板与规则注入策略（固定 schema 输出）
  - [x] P0-W2：前端新增“实施评估预览弹窗”（展示+异常提示）
  - [x] P0-W2：完成预览结果映射并回填实施评估页面（不直接覆盖正式版本）
  - [x] P0-W2：补充构建验证与关键用例（成功/超时/结构异常）

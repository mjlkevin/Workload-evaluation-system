...
> **实现现状注记（2026-04）**：后端为 **Express 单入口**（`apps/api/src/main.ts`），前端统一为 **Next.js**（`ui/V0_SAAS`）；【历史说明，已下线】`apps/web`。完整差异见 `03_技术设计/系统演进/实现与文档对齐说明.md`。
...
- **当前实现**：Next.js（`ui/V0_SAAS`）+ React 组件为主；下一代沙箱 Vite + React（`ui/V2_PROTOTYPE`）已启动 Phase B。设计规范以 `ui/WES 优化方案 · 工程落地版.html` 为准。

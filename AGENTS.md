...
- **前端样式体系约定（2026-08-25 架构裁决，阶段 3 及以后长期生效）**：
  - **新代码统一 Tailwind + tokens 桥接**：新组件默认用 `src/tailwind.css` `@theme` 桥接出的 OKLCH 语义类名（`bg-ink`、`text-ink-3`、`rounded-md` 等），tokens 变量已全部桥接，不产生第二套色值；preflight 已关闭、纯增量叠加零回归。内联 style 仅用于运行时动态值（如逐元素 animationDelay）。
  - **旧代码跟随业务改动顺手迁移，不做大爆炸**；在此之外允许对存量内联 style 按高频模式做机械替换（预估消 30–40%），不单独排期。
  - **`components.css` / `layout.css` 冻结扩增、不退役**（约 671 处 BEM 类名引用在役，全量迁移与阶段 2/3 排期冲突）；退役评估放阶段 3 后段单独立项。
  - **内联样式处置优先级**：P0（视觉 bug / 失序源）→ P1（高频组件）→ P2（低频页面）→ P3（运行时动态值，保留内联）。

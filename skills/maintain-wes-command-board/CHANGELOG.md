...
- `references/board-readability-spec.md` §3 数字单一来源补一条**统计枚计数口径**：页顶 pill 的计数一律由表结构实取单一来源定值（总数 = `tbody tr` 含 `td` 的行数 + ID 去重校验；分项逐行归桶且不得混用中 / 英文标签口径），写完必须做桶闭合校验，禁止用“推断哪一个多了表头”反向修正旧口径。源于本批两次计错自取回：`issues.html` pill 首次补正写成 30 / 2 / 19 / 7，实际应为 31 / 2 / 19 / 8（数据行误判 30；“转需求 · RP-” 手工计 6 实为 7、漏 RP-059），修正后由 headless DOM 逐行归桶与源码正则计数两种独立方法交叉实取一致。

...
Box stroke:     #4a4a4a  (dark gray, flat default for sparse diagrams)
Box radius:     12px
Text primary:   #1a1a1a  (near black)

Same-hue softened stroke variants (preferred for warm multi-node compositions;
the flat #4a4a4a outline renders harsh when many colored containers share one canvas):

```
Semantic   Fill       Stroke     Text (title / body / caption)
Teal       #9dd4c7    #6fb3a5    #2e4b44 / #3d5a52 / #4d6b62
Blue       #a8c5e6    #7fa3cc    #2c405c / #3a4f6e / #4a5f7e
Beige      #f4e4c1    #d9c49a    #5c4a26 / #6e5a33 / #7d6a42
Gray       #e8e6e3    #b9b3a9    #453f37 / #57514a / #6b665e
Sub-chip   #fffdf9    (parent hue at 40%)  #6b665e body text
```

Shadow (calibrated for cream background — stdDeviation=6 is nearly invisible):
...
Code identifiers (file paths, API routes, state names) pair a monospace layer:

```
font-family: 'JetBrains Mono', ui-monospace, SFMono-Regular, Menlo, monospace
font-size:   12px identifiers, 8.5-9.5px sub-chip text (only when rendered ≥2x / viewed ≥1280px)
```
...
**Halo technique** (preferred when a label must sit on or near a connector channel):
render the text with a canvas-colored stroke behind the fill instead of adding
a background rect — cheaper, and it never misaligns on zoom:

```xml
<text x="..." y="..." fill="#6b665e" font-size="12" text-anchor="middle"
      stroke="#f8f6f3" stroke-width="4" paint-order="stroke">persist</text>
```
...
## Sub-Component Chips (Container + Chip Pattern)

For detailed architecture diagrams where each layer component decomposes into
sub-components, use the container+chip pattern instead of nested full nodes:

```xml
<!-- Parent container: semantic fill + same-hue soft stroke -->
<rect x="620" y="360" width="560" height="280" rx="12"
      fill="#a8c5e6" stroke="#7fa3cc" stroke-width="2" filter="url(#shadow-soft)"/>
<text x="900" y="386" text-anchor="middle" fill="#2c405c" font-size="15" font-weight="700">modules/harness</text>

<!-- Sub-component chip: warm white, 1px same-hue stroke, rx=6 -->
<rect x="640" y="400" width="160" height="22" rx="6"
      fill="#fffdf9" stroke="#7fa3cc" stroke-width="1"/>
<text x="720" y="415" text-anchor="middle" fill="#2c405c" font-size="11"
      font-family="'JetBrains Mono', ui-monospace, monospace">harness.controller</text>
```

Rules:
- Internal chains between chips use thinner arrows (`stroke-width="1.2"`); external connectors anchor on container edges only
- Chip text 8.5-11px is acceptable at ≥1280px viewBox width; container titles stay ≥13px
- State machines / key flows can be shown as a full-width chip strip inside the container
- Run `scripts/check-geometry.py` before delivery — 0 crossings, 0 penetrations
...
The example below uses single straight lines for simplicity. Real diagrams with
more than 6 connectors MUST follow the **Orthogonal Routing Algorithm** in
SKILL.md (channel layering, port selection, topology gate) — copying these
straight center lines verbatim produces crossings.

...
2. **Extract structure** — identify layers, nodes, edges, flows, and semantic groups from user description. When diagramming an existing codebase, component names, module paths, and state names MUST be extracted from the actual source tree (e.g. `ls modules/<domain>/`, grep the state vocabulary) — never invent identifiers
...
9. **Validate**: Run `"$SKILL_ROOT/scripts/validate-svg.sh" file.svg` to check XML, markers, geometry, composition budgets, and renderability. For diagrams with orthogonal connectors, additionally run `python3 "$SKILL_ROOT/scripts/check-geometry.py" file.svg` — it fails closed on line-line crossings and connector-through-node penetrations (see **Orthogonal Routing Algorithm** below)
...
12. **Visual review gate** — if your runtime can read images, load the exported PNG back and inspect it. Syntactic validity does not guarantee visual correctness: arrows may cross through component interiors, labels may collide with lifelines or other labels, boxes may overlap, alt-frame text may sit on top of a message, or a legend may cover content. If you see any of these, revise the SVG and re-export, with at most two focused correction passes. Common fixes:
  ...
  **Verification fallback matrix** (field-tested when the primary image path is unavailable): prefer programmatic geometry assertions over eyeballing — (1) run `check-geometry.py` offline, or assert in-browser via `evaluate_script` (node/container counts, connector penetrations, H×V crossings, horizontal overflow, text-bbox overlaps); (2) if screenshot/export tools time out, render the target element in-page with html2canvas-pro from CDN and download the PNG; (3) macOS zero-dependency fallback: `qlmanage -t -s <N> -o <dir> file.svg` — note thumbnails are square-cropped, so pair this with the programmatic checks.
...
- **ViewBox scaling**: when a diagram exceeds 12 nodes or uses nested containers, expand to `0 0 1280 H`; estimate `H = layers × layer_height + 120` (legend/footer). Increase canvas size instead of packing elements tighter
- **Container + chip pattern** (detailed sub-components): draw each parent component as a semantic-colored container rect (rx=12), and its sub-components as white `#fffdf9` chips (rx=6, 1px same-hue stroke) inside the container; route internal chains with thinner arrows (stroke-width ≈1.2) and anchor external connectors on container edges only
...
**Orthogonal Routing Algorithm** (constructive — apply whenever a diagram has more than 6 connectors):
1. Use only `M`/`H`/`V` segments; no diagonals (`L`/`C`) on inter-layer connectors
2. **Port selection**: exit/enter on node edges; the x of a vertical drop segment MUST fall within the target node's top-edge span (mirrored for horizontal entry into a side edge)
3. **Channel layering**: place parallel connectors' horizontal segments on distinct y channels (≥10px apart); keep vertical drops parallel and non-overlapping
4. Reserve a clear corridor between layers (≥40px) for horizontal channels; never place a channel inside a node row
5. **Fail-closed topology gate**: run `scripts/check-geometry.py` — crossings and penetrations must both be 0 before delivery; reserve jump-over arcs for genuinely unavoidable crossings in non-orthogonal diagrams only
...
10. **Topology Gate** (orthogonal diagrams): `scripts/check-geometry.py` MUST report 0 line-line crossings and 0 connector-through-node penetrations (sampled with ≥3px inset). Note: `validate-svg.sh`'s collision heuristic can false-positive on intra-container chip arrows in container+chip diagrams — for such diagrams, `check-geometry.py` is the authoritative gate
11. **Halo Labels**: labels that must sit on or near a line use `stroke="<canvas-bg>" stroke-width="3-4" paint-order="stroke"` instead of extra background rects
...
- Text: minimum 12px, prefer 13-14px labels, 11px sub-labels, 16-18px titles. Exception: high-density container+chip diagrams may use 8.5-9.5px chip text when the deliverable is rendered at ≥2x or viewed at ≥1280px width; titles and layer labels still stay ≥11px
- Code identifiers (file paths, API routes, state names): prefer a monospace stack (`'JetBrains Mono', ui-monospace, monospace`) paired with the style's sans stack
...
### Site Embedding Mode (static-site / dashboard integration)

When the deliverable must live inside an existing static site (e.g. a project dashboard page):
1. Keep the standalone SVG asset at a stable path (provenance + reuse), and inline-embed the same bytes into the target HTML panel
2. Wrap it in a responsive container: `.diagram { overflow:hidden; border:1px solid var(--line); border-radius:10px }` with `.diagram svg { display:block; width:100%; height:auto }`
3. If the site has a build step (nav injection, CSS merge), register the page in the nav config, rebuild, and run the site's consistency check
4. Verify the embedded bytes equal the source asset via a line-by-line normalized (stripped) diff — never raw string equality after re-indentation
5. Record the change in the site's changelog/timeline page with evidence (build output, topology-gate results)
...
**Style 6 field notes** (from production use on warm-cream architecture diagrams): prefer the same-hue softened strokes over flat `#4a4a4a`, the calibrated soft shadow (`dy=2 stdDeviation=3 flood-opacity=0.10`), halo labels, and the container+chip pattern — upgraded tokens are in `$SKILL_ROOT/references/style-6-claude-official.md`.

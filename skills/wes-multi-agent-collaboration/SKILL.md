...
- **凭据与模型两条硬性前置**：token 不得出现在对话里（存 `~/.qoder-env` 并 `source` 取用），机械任务显式指定廉价模型。两条都属于"漏了不会报错、只会事后付账"的类型——凭据漏了是明文落盘且出网，模型漏了是同一批活贵数倍。具体取用方式与核实办法见本节末尾指向的第七节，此处不复述，避免两份规则各自漂移。

...
# Ensure 3000/3002 are available.
...
npm install --prefix ui/V2_PROTOTYPE
...
WEB_STATUS="$(curl --noproxy "*" -sS -m 12 -o /dev/null -w "%{http_code}" "http://localhost:3002" || true)"
...
echo "[recover] web ready: http://localhost:3002"
echo "[recover] logs:"
echo "  - ${API_LOG}"
echo "  - ${WEB_LOG}"

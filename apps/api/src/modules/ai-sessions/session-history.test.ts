...
//
// 本文件是这道「不变」的**确定性**证据：断言对象是合成样本，不依赖库里恰好有什么数据，
// 因此在 CI（空测试库）与本地（开发库 100 条）都同样成立。真实数据上的同一对照由
// scripts/session-history-derive-parity.ts 另取一次证据，两者互补：
//  · 本文件锁形状（含空历史、system/tool 角色、悬挂附件引用、窗口边界、判据形状异常）；
//  · 脚本锁实存（开发库全部会话，逐条）。
// 之所以不能只靠脚本：开发库 100 条会话里绝大多数 messages 为空数组，
